...
def eval_vatex_en_r(submit_dir, truth_dir, output_dir, val_only=True):
    dataset_name = "vatex_en_r"
    print("Evaluating task {}".format(dataset_name))

    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    if dataset_name == "vatex_en_r":
        output_path = join(
            output_dir, "{}_metrics.json".format(dataset_name))
        file_paths = dict(
            val=dict(submission=join(submit_dir, "vatex_en_r_val_predictions.json"),
                     solution=join(truth_dir, "vatex_en_r_val_release.jsonl"),
                     output=join(output_dir, "vatex_en_r_val_metrics.json")),
        )
        if not val_only:
            file_paths.update(
                test_public=dict(submission=join(submit_dir, "vatex_en_r_test_public_predictions.json"),
                                 solution=join(truth_dir, "vatex_en_r_test_public_gt.jsonl"),
                                 output=join(output_dir, "vatex_en_r_test_public_metrics.json"))
            )
        video2dur_idx_path = join(truth_dir, "vatex_en_r_video2dur_idx.json")
    else:
        raise ValueError
...
if __name__ == '__main__':
    ...
    eval_vatex_en_r(submit_dir, truth_dir, output_dir)
